document.addEventListener('DOMContentLoaded', () => {
    ...
    // 1. Dynamic Flashlight Hover Effect
    ...
    panels.forEach(panel => {
        panel.addEventListener('mousemove', (e) => {
            ...
            // Updates the background to create a soft light following the cursor
            panel.style.background = `
                radial-gradient(circle at ${x}px ${y}px, 
                rgba(255, 255, 255, 0.06), 
                rgba(20, 20, 25, 0.4) 40%)
            `;
            panel.style.borderColor = 'rgba(255, 255, 255, 0.15)';
        });
        
        panel.addEventListener('mouseleave', () => {
            // Reverts to the default glass state
            panel.style.background = 'rgba(20, 20, 25, 0.4)';
            panel.style.borderColor = 'rgba(255, 255, 255, 0.08)';
        });
    });

    // 2. Data Obfuscation / Decryption Interaction
    const obfuscatedElements = document.querySelectorAll('.obfuscated');
    
    obfuscatedElements.forEach(el => {
        // Store the original masked text (the asterisks)
        const maskedText = el.innerText;
        // Grab the real data from the HTML data attribute
